...
from ..common.optim import CLAMB, TurboAdaMuon, TrapezoidalWarmupLR
...
WARMUP_STEPS = 500
WARMPUP_LR = 1e-7
ADAMW_START_LR = 2e-3 * math.sqrt(BS / 256)
ADAMW_END_LR = 3e-4
MUON_START_LR = 0.02 * math.sqrt(BS / 256)
MUON_END_LR = 2e-3
EPOCHS = 10
...
def run():
  ...
  dataloader = Dataloader({
    ...
    "y": BatchDesc(shape=(11,), dtype=dtypes.float32),
  ...
  _NODECAY_KW = ("norm", "gamma", "beta", "bias", "ls", "_token", "pos_emb")
  ...
  muon_optim    = Muon(muon_params, lr=MUON_START_LR, weight_decay=0.01)
  adamw_decay   = CLAMB(decay_params, lr=ADAMW_START_LR, weight_decay=0.01, adam=True)
  adamw_nodecay = CLAMB(nodecay_params, lr=ADAMW_START_LR, weight_decay=0.0, adam=True)
